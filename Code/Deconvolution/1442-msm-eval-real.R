...
log_run_header("1442-msm-eval-real.R", opt) #Header log
...
get_warmstart <- function(ins_choice, S = opt$S, data = stage2_data) {
    ...
    ## TEST INVERSION at fixed candidate points (2026-09-19): J(theta_0) = n_eff * gmm_obj(theta_0) vs chi2_{d_g_eff}
    pts <- list(ELVIS = c(5.427e-7, 3.46356, 4.3, 0.54), MSL = c(2.724114e-5, 7.417881, 3.243078, 0.233465), MSM_best = c(6.847224e-4, -5.423294, 14.9643, 3.424686))
    J_pts <- t(sapply(pts, function(p) { gb <- build_moments(p); ev <- eigen(cov(gb), symmetric = TRUE)$values; c(J = nrow(gb) * gmm_obj(p), d_g_eff = sum(ev > 1e-8)) }))
    J_pts <- cbind(J_pts, crit_95 = qchisq(0.95, J_pts[, "d_g_eff"]), reject = J_pts[, "J"] > qchisq(0.95, J_pts[, "d_g_eff"]))
    cat("\nNAIVE MSM, test inversion at candidate points (real data):\n"); print(round(J_pts, 2))
    list(
        J_pts       = J_pts,
...
if (opt$ins_only == "") {
    ...
    out_file <- sprintf("Code/Products/1442-MSM-eval-%s-S%d%s.RData", opt$ins_only, opt$S, lnM_tag)
