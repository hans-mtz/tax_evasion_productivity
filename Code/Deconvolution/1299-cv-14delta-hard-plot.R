## Final standard AK2020-style CI plot, HARD (absolute Theorem F.1) test,
## CV-adjusted R, all 14 Delta points (-7% to +5%), after the Step-2
## hard-test-targeted fine bisection. Bound convention: outermost TESTED
## point that passes (matches this project's standard, and AK2020's own
## F_figure1.R) -- not a further-interpolated "true" crossing (see
## Research-log/log.md for the -6%/-7% resolution-precision caveat found
## while investigating this). 2026-09-13.
...
df <- read.csv("Code/Products/1299-cv-all-final-combined.csv") %>%
    mutate(R_round = round(R, 4)) %>% distinct(Delta, R_round, .keep_all = TRUE) %>% select(-R_round)

df <- df %>% mutate(TS = 2 * n * Lhat, pass = TS <= qc)
...
## Baseline (Delta=0) reference band, to make "distinguishable from baseline"
## visually obvious across the whole plot
...
p <- ggplot() +
    ...
    labs(x = expression(paste("Purchases-tax shifter ", Delta)), y = "Real mean revenue per firm-period, R (COP)",
         title = "CV-adjusted R, HARD test (Theorem F.1) — all 14 Δ points, fully bisected",
         subtitle = paste0("Blue = passes at 95% (χ²₁₀=", round(qc, 1),
                            "); gray × = rejected. Gray band = Δ=0's own passing interval, for visual reference."),
         caption = "Cuts up to -5% still overlap the Delta=0 band; -6%/-7% come close but the true crossing is not yet pinned exactly (see chat/log). Increases (>=+0.5%) separate cleanly.") +
...
ggsave("Paper/images/1299-cv-14delta-hard.png", p, width = 11, height = 7.5, dpi = 300)
cat("Saved: Paper/images/1299-cv-14delta-hard.png\n")
