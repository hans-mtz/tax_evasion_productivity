...
## test-inversion, moment set A (9 rows), trim=0.5%, lag_m, C++ deltagrid
## mode (2026-09-08). 3D perspective surface (base R persp(), the 7x7 grid
## is exactly regular so no interpolation is needed) with a translucent
## threshold plane at Lhat_min + chi2_{9,.95}/(2n) -- points below the plane
## are inside the 95% confidence region, mirroring Schennach's own style of
## showing the CUE objective surface with the confidence-region cutoff.

df <- read.csv("Code/Products/1240-deltagrid-lag_m-combined.csv")
...
png("Paper/images/1241-stage2-deltagrid-lag_m-3d.png", width = 2400, height = 1800, res = 220)
par(family = "Times")
res <- persp(d1s, d2s, Z, theta = -35, phi = 22, expand = 0.6, col = "lightgrey",
             ...
             main = "Stage-2 ELVIS: L_n(delta1,delta2), lag_m, moment set A (9 rows), trim=0.5%",
...
corners <- expand.grid(d1 = range(d1s), d2 = range(d2s))
...
dev.off()
cat("Saved: Paper/images/1241-stage2-deltagrid-lag_m-3d.png\n")
