...
p <- ggplot(df, aes(x = lambda, y = Lhat)) +
    ...
    labs(x = expression(lambda~"(log scale)"), y = expression(hat(L)[n]~"(log scale)"),
         ...
         subtitle = "Combined coarse+fine, 16 pts (deduped), free (delta0,delta1,delta2,gamma). Dashed=soft threshold, dotted=hard threshold.") +
    theme_minimal() +
    theme(
...
ggsave("Paper/images/1272-lambdagrid-final-lhat-lambda.png", p, width = 9.5, height = 7, dpi = 300)
cat("Saved: Paper/images/1272-lambdagrid-final-lhat-lambda.png\n")
