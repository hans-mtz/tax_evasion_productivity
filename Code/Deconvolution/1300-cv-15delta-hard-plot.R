...
p <- ggplot() +
    ...
    labs(x = expression(paste("Purchases-tax shifter ", Delta)), y = "Real mean revenue per firm-period, R (COP)",
         title = "CV-adjusted R, HARD test (Theorem F.1) — all 15 Δ points, fully bisected",
         subtitle = paste0("Blue = passes at 95% (χ²₁₀=", round(qc, 1),
                            "); gray × = rejected. Gray band = Δ=0's own passing interval, for visual reference."),
         caption = "Crossing found: Delta=-8% is the smallest tax cut statistically distinguishable from Delta=0 (sequential search, see Research-log/log.md). Cuts -1% to -7% still overlap the Delta=0 band. Increases (>=+0.5%) separate cleanly.") +
...
ggsave("Paper/images/1300-cv-15delta-hard.png", p, width = 11, height = 7.5, dpi = 300)
cat("Saved: Paper/images/1300-cv-15delta-hard.png\n")
