...
df <- read.csv("Code/Products/1264-revenue-baseline-finegrid-lag_m.csv")
...
p <- ggplot(df, aes(x = Delta, y = mean_R_real)) +
    ...
    labs(x = expression(Delta~"(change in purchases-side tax rate)"),
         ...
         subtitle = "No optimization -- fixed (lambda,delta0,delta1,delta2,eta,gamma) at the Phase-0 point. lambda=1.3e-7.") +
    theme_minimal() +
    theme(
...
ggsave("Paper/images/1264-revenue-finegrid-full.png", p, width = 9, height = 6, dpi = 300)
...
ggsave("Paper/images/1264-revenue-finegrid-zoom.png", p2, width = 9, height = 6, dpi = 300)

cat("Saved: Paper/images/1264-revenue-finegrid-full.png\n")
cat("Saved: Paper/images/1264-revenue-finegrid-zoom.png\n")
