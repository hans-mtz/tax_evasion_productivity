...
fmls <- c(
    ...
    "cal_V ~ -1 + i(lab_lag_ntile)",
    "cal_V ~ -1 + i(cap_lag_ntile)",
    "cal_V ~ -1 + i(lab_lag2_ntile)",
    "cal_V ~ -1 + i(cap_lag2_ntile)",
...
fmls_ols <- list(
    ...
    c("cal_V", "lab_lag_ntile", "lab_lag_ntile"),
    c("cal_V", "cap_lag_ntile", "cap_lag_ntile"),
    c("cal_V", "lab_lag2_ntile", "lab_lag2_ntile"),
    c("cal_V", "cap_lag2_ntile", "cap_lag2_ntile"),
...
save(
    ...
    file = "Code/Products/930.5-boot-se-het.RData"
)

## %% Tables and Plots 

load("Code/Products/930.5-boot-se-het.RData")
...
dec_aux_df %>% 
    filter(
        Model %in% c("OLS: Labour (t-1)", "OLS: Capital (t-1)")
    ) |>
    ggplot(aes(x = Decile, y = Coefficient, group = Model, color = Model)) +
    facet_grid(~Model, axes = "all") +
    # geom_line() +
...
dec_aux_df %>% 
    filter(
        Model %in% c("OLS: Labour (t-2)", "OLS: Capital (t-2)")
    ) |>
    ggplot(aes(x = Decile, y = Coefficient, group = Model, color = Model)) +
    facet_grid(~Model, axes = "all") +
    # geom_line() +
