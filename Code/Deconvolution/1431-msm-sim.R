...
DEFAULTS <- list(
    ...
    ins_only = "",
    wmode = "none",  # "none": plain naive MSM (Tim); "omegaA": importance-weight each eps draw by f_omega(omega_hat) (unconditional normal, moment-deconvolved from W on all firms)
...
log_run_header("1431-msm-sim.R", opt) #Header log

## ---- SIMULATED-DATA VERSION (2026-09-19): same MSM code as 1201-MSM.R, applied to the model-faithful DGP (1410-dgp-model.R) ----
## stage2_data / eps_targets are built from the simulated panel with the SAME sample rules as 1411-sim-export.R (lag available,
## unincorporated tau_P>0 interior with top 0.5% by M* dropped; corps kept for the eps pool, epsilon = -V for corps since e=0).
## Truth: lambda=1e-4, delta=(3.837,3.5,0.5), sigma_psi=0.66.
sim <- readRDS("Code/Products/msl/1410-sim-base-lam1e-4-s070.rds"); sp <- sim$spec
dd <- sim$data %>% dplyr::filter(!is.na(lag_mstar))
cut <- quantile(dd$xstar[!dd$corp & dd$tauP > 0], 1 - 0.005)
dd <- dd %>% dplyr::filter(corp | tauP == 0 | xstar <= cut)
stage2_data <- dd %>% transmute(ins = "lag_m", sic_3 = sector, corp = corp, epsilon = ifelse(corp, -V, NA_real_),
                                cal_V = V, tilde_cal_W = Wt, M_star = xstar, sales_tax_rate_purchases = tauP, beta = sp$beta)
eps_targets <- stage2_data %>% dplyr::filter(corp) %>% group_by(ins, sic_3) %>% summarise(n = n(), .groups = "drop") %>% mutate(pooled_fallback = n < 30)
cat(sprintf("sim: %d rows, %d corp, %d unincorp interior; truth lambda=%g d0=%g d1=%g d2=%g\n", nrow(stage2_data), sum(stage2_data$corp),
            sum(!stage2_data$corp & stage2_data$sales_tax_rate_purchases > 0), sp$lambda, sp$d0, sp$d1, sp$d2))
...
get_warmstart <- function(ins_choice, S = opt$S, data = stage2_data) {
    ...
    ## importance weights w = f_omega(omega_hat) (theta-free): the naive draw eps ~ f_eps (feasibility-truncated) is reweighted toward
    ## the posterior p(eps | data) ∝ f_eps(eps) f_omega(W - bo*eps) [f_psi and the Jacobian are still missing -- that is MSL].
    bo_m <- 1 - mean(unincorp$beta); mu_e <- mean(pooled_pool); v_e <- var(pooled_pool)
    mu_om_w <- mean(run_data$tilde_cal_W) - bo_m * mu_e; sd_om_w <- sqrt(max(var(run_data$tilde_cal_W) - bo_m^2 * v_e, 0.05 * var(run_data$tilde_cal_W)))
    w_draw <- if (opt$wmode == "omegaA") dnorm(stacked$omega_hat, mu_om_w, sd_om_w) else rep(1, nrow(stacked))
    cat(sprintf("  wmode=%s  f_omega: mean %.3f sd %.3f; weights: ESS/n = %.3f\n", opt$wmode, mu_om_w, sd_om_w, sum(w_draw)^2 / sum(w_draw^2) / length(w_draw)))
    ...
    build_moments <- function(par) {
        ...
        g_sum_by_firm <- rowsum(g * w_draw, group = firm_id)
        w_by_firm     <- as.vector(rowsum(w_draw, group = firm_id))
        keep <- w_by_firm > 0
        (g_sum_by_firm / w_by_firm)[keep, , drop = FALSE]   # one row per firm: that firm's own (weighted) S-draw average
    }
    ...
    n_eff       <- nrow(g_by_firm_f)   # firms actually contributing a valid
                                         ...
                                         # feasibility filter)
    df_test     <- d_g_eff   # TEST INVERSION: theta is fixed at each candidate, so the limit is chi2_{d_g}, NOT chi2_{d_g - d_theta}
    par_true <- c(sp$lambda, sp$d0, sp$d1, sp$d2)
    L_true   <- gmm_obj(par_true)

    J_stat <- n_eff * fit$value; J_true <- n_eff * L_true   # n = n_eff (firms), NOT n_firms*S: S only
    ...
    rej_rule <- sprintf("TEST INVERSION vs chi2_{%d,.95}=%.3f:  at fitted theta: J=%.1f (%s);  at the TRUE theta: J=%.1f (%s)",
                        df_test, crit, J_stat, ifelse(J_stat > crit, "reject", "fail to reject"), J_true, ifelse(J_true > crit, "reject", "fail to reject"))

    list(
        par         = c(fit$par, sigma2_psi = sigma2_psi_init),
        L_n         = fit$value,
        J_stat      = J_stat, J_true = J_true,
        d_g_eff     = d_g_eff,
        df_test     = df_test,
        n_eff       = n_eff,
        rej_rule    = rej_rule,
...
run_lag2W  <- FALSE   # sim has a single (lag_m-style) sample
...
if (opt$ins_only == "") {
    save(warmstart_lag_m, file = sprintf("Code/Products/msl/1431-MSM-sim-%s%s.RData", opt$wmode, lnM_tag))
} else {
    out_file <- sprintf("Code/Products/msl/1431-MSM-sim-%s-S%d%s.RData", opt$wmode, opt$S, lnM_tag)
