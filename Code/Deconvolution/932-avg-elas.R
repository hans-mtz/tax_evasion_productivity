...
fmls_nc <- c(
    "cal_V ~ l_pct + l_pct^2 + k_pct + k_pct^2 + exports_pct + exports_pct^2 + imports_pct + imports_pct^2 | rev_pct + rev_pct^2 ~ lag_rev_pct + lag_rev_pct^2",
    "cal_V ~ l_pct + l_pct^2 + k_pct + k_pct^2 + exports_pct + exports_pct^2 + imports_pct + imports_pct^2 | rev_pct + rev_pct^2 ~ cap_l_pct + cap_l_pct^2",
    "cal_V ~ l_pct + l_pct^2 + k_pct + k_pct^2 + exports_pct + exports_pct^2 + imports_pct + imports_pct^2 | rev_pct + rev_pct^2 ~ lab_l_pct + lab_l_pct^2",
    "cal_V ~ lag_rev_pct + lag_rev_pct^2 + l_pct + l_pct^2 + k_pct + k_pct^2 + exports_pct + exports_pct^2 + imports_pct + imports_pct^2"
)
...
coef_elas_iv <- list(
    "fit_rev_pct" = c("fit_rev_pct", "fit_I(rev_pct^2)"),
    "l_pct" = c("l_pct", "I(l_pct^2)"),
    "k_pct" = c("k_pct", "I(k_pct^2)"),
    "exports_pct" = c("exports_pct", "I(exports_pct^2)"),
    "imports_pct" = c("imports_pct", "I(imports_pct^2)")
)
names(coef_elas_iv) <- var_elas_iv
coef_elas_ols <- list(
    "lag_rev_pct" = c("lag_rev_pct", "I(lag_rev_pct^2)"),
    "l_pct" = c("l_pct", "I(l_pct^2)"),
    "k_pct" = c("k_pct", "I(k_pct^2)"),
    "exports_pct" = c("exports_pct", "I(exports_pct^2)"),
    "imports_pct" = c("imports_pct", "I(imports_pct^2)")
...
comb_vars <- data.frame(
    ...
    mdl = c(rep("IV: Lagged Revenue", length(var_elas_iv)),
            rep("OLS: Lagged Revenue", length(var_elas_ols))
    )
)
...
# OLS Elasticity of Revenue (t-1)
allcoefs <- reg_mrg_ls[["OLS: Lagged Revenue"]] |> coef()
allcoefs[coef_elas_ols[["Revenue (t-1)"]]]
tmp_df <- df %>% filter(corp == "Other")
avg_elas <- model.matrix(
    fmls_elas_ols[["Revenue (t-1)"]] |> as.formula(),
    data = tmp_df
) %*% allcoefs[coef_elas_ols[["Revenue (t-1)"]]] |> mean()

model.matrix(
    "~I(2*l_pct)"|> as.formula(),
    data = tmp_df
) |> head()

# IV Elasticity of Revenue
allcoefs_iv <- reg_mrg_ls[["IV: Lagged Revenue"]] |> coef()
allcoefs_iv[coef_elas_iv[["Revenue"]]]
tmp_df$fit_rev_pct <- reg_mrg_ls[["IV: Lagged Revenue"]]$iv_first_stage$`rev_pct` |> predict(sample="original")
avg_elas <- model.matrix(
    fmls_elas_iv[["Revenue"]] |> as.formula(),
    data = tmp_df
) %*% allcoefs_iv[coef_elas_iv[["Revenue"]]] |> mean()

## Defining function to compute average elasticity

compute_avg_elas <- function(size_v, mdl, reg_ls, dta){
    tmp_dta <- dta %>% filter(corp == "Other")
    coefs_tmp <- reg_ls[[mdl]] |> coef()
    coef_elas_tmp <- if(mdl == "IV: Lagged Revenue"){
        coef_elas_iv[[size_v]]
    } else {
        coef_elas_ols[[size_v]]
    }
    # print(coefs_tmp[coef_elas_tmp])
    if(any(is.na(coefs_tmp[coef_elas_tmp]))){
        stop(paste("Coefficients for", size_v, "not found in model", mdl))
    }
    fml_elas_tmp <- ifelse(mdl == "IV: Lagged Revenue",
        fmls_elas_iv[[size_v]],
        fmls_elas_ols[[size_v]]
    )
    # print(fml_elas_tmp)
    if (mdl == "IV: Lagged Revenue" & size_v == "Revenue"){
...
compute_avg_elas("Labour", "IV: Lagged Revenue", reg_mrg_ls, df)
...
load("Code/Products/932-avg-elas.RData")
...
save(
    ...
    elas_aux_df,
    coef_aux_df,
    file = "Code/Products/932-avg-elas.RData"
)
...
render_tbl(
    ...
    x_desc = "var_desc",
    order_vec = var_desc_ltx,
    aux_df = coef_aux_df %>% filter(model %in% c("IV: Lagged Revenue", "OLS: Lagged Revenue")),
    file_name = "932-coefs" 
)
