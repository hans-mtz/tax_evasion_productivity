...
get_beta_diff_poly <- function(
    ...
    model <- data_temp %>%
        ...
        fixest::feols(
            log_share ~ compliers + polym(m, k, l, degree = d, raw = TRUE),
            data = .
        )
...
beta_diff_poly <- do.call(rbind, beta_diff_poly_list)

## Ordering the DF -------
beta_diff_poly <- beta_diff_poly[do.call(order, beta_diff_poly), ]

# Saving results ---------------

save(
    beta_diff_poly,
    file = paste0(products_dir, "beta_diff_poly.RData")
)
