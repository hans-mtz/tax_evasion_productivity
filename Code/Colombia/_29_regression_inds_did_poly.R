...
get_inds_did_poly <- function(
    ...
    model <- data_temp %>%
        ...
        fixest::feols(log_share ~ compliers*inds + polym(m, k, l, degree = d, raw = TRUE), data = .)
...
beta_inds_did_poly_list <- lapply(seq_len(nrow(combinations)), function(i) {
    with(
        combinations[i, ],
        get_inds_did_poly(
            o = "sales",
...
beta_inds_did_poly <- do.call(rbind, beta_inds_did_poly_list)

## Ordering the DF -------
beta_inds_did_poly <- beta_inds_did_poly[do.call(order, beta_inds_did_poly), ]

# Saving results ---------------

save(
    beta_inds_did_poly,
    file = paste0(products_dir, "beta_inds_did_poly.RData")
)
