...
local inter_shares share mats_share ded_i_share
local inter_names gnr_ii materials dedudctibles
...
// 	keep if j_org == 3 /* Corporations only */
...
// 		matrix R_mat`mat_i' = J(1,7,0) //  m,k,l,bigE,mean si_level, mean error, sd error
		...
		matrix R_mat`mat_i' = J(1,3,0) //  m,bigE,mean si_level, N_drop
		
		keep if si_level > $g_cut & l_rgo~=. & l_rk ~=. & l_l ~=. & l_rii~=.  & j_org == 3 
...
// 		mat R_mat`mat_i'[1,4] = `N_drop'/r(N)

// 		mat R_mat`mat_i'[1,2] = beta

		sum mexp_eg
		mat R_mat`mat_i'[1,2] = r(mean)
...
matrix colnames all_inds = m bigE err_sd
