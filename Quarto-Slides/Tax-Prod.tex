...
moments (\(d_g=9\)), no nuisance parameters beyond \(\gamma\):
...
\item
  ...
  \(\hat\Omega^-\) an eigenvalue-truncated generalized inverse --- same
...
\item
  ...
  reseed
\item
  \textbf{Solver}: Nelder-Mead (beats BOBYQA decisively on this
  problem), unbounded \(\gamma\) (a large/degenerate \(\gamma\) is an
  expected solution, Schennach 2014 --- not a numerical failure),
  two-pass refinement
\item
  \textbf{Seeding}: every \(\lambda\) grid point seeded independently
  from the same validated starting point --- no chaining across grid
  points
\end{itemize}
\end{frame}
...
\begin{frame}{\(\hat L_n(\lambda)\) --- Both Instruments}
\protect\phantomsection\label{hat-l_nlambda-both-instruments}
\begin{center}
\includegraphics[width=\linewidth,height=0.75\textheight,keepaspectratio]{../Paper/images/1285-lambdagrid-slide.png}
\end{center}
\end{frame}
...
\tightlist
\item
  \(m^*_{it-1}\): a clean V-shape, genuine minimum, real rejection on
  both sides --- a proper confidence interval
\item
  \(\tilde{\mathcal W}_{it-2}\): also rejects everywhere except two
  points --- its own interior minimum, and separately the smallest
  \(\lambda\) we tested. That second pass just means: for this
  instrument, we can't rule out arbitrarily weak detection either --- we
  only get an upper bound on \(\lambda\) from it, not a full interval.
  Consistent with it being the weaker of the two instruments overall.
\end{itemize}
\end{frame}
...
\midrule\noalign{}
\endhead
\(m^*_{it-1}\) & \(3.50\times10^{-7}\) & \([2.26,5.43]\times10^{-7}\) &
0.000387 & 3.459 & 4.315 & \textbf{0.541} & 3.99 & 0.64 \\
\(\tilde{\mathcal W}_{it-2}\) & \(5.43\times10^{-7}\) &
\(5.43\times10^{-7}\) (point) & 0.000401 & 10.206 & 7.460 &
\textbf{0.972} & 3.84 & 1.86 \\
\bottomrule\noalign{}
\end{longtable}
}

\(\hat\delta_2>0\) for both --- the cost function opens the correct
(U-shaped) way. \(\hat\omega^*\) lands in a similar plausible range for
both, even though the individual \(\hat\delta_1,\hat\delta_2\) differ
--- expected: the two instruments come from different first-stage
residuals, so they carry somewhat different implied productivity
distributions.
\end{frame}

\begin{frame}{A Data-Side Check on \(\hat\lambda\): What Detection
Probability Does This Imply?}
\protect\phantomsection\label{a-data-side-check-on-hatlambda-what-detection-probability-does-this-imply}
Does \(\hat\lambda\) imply a sane detection probability at a typical
evading firm? Quick check by forward-simulating each firm's own evasion
at the fitted \((\hat\theta,\hat\gamma)\) and reading off the population
mean/median --- \textbf{a ballpark reading, not the formal way to pin
this down} (that would need its own auxiliary moment, jointly
re-estimated with everything else, not a post-hoc average):
...
\tightlist
\item
  \(E[e]\gg\text{Med}[e]\) for both --- a right-skewed evasion
  distribution: most firms evade modestly, a long right tail of
  aggressive evaders pulls the mean well above the median
\item
  Both implied detection probabilities are small, well under the 50\%
  FOC ceiling --- no sign of \(\hat\lambda\) pushing anyone near the
  boundary
\end{itemize}
\end{frame}
...
\tightlist
\item
  \textbf{\(m^*_{it-1}\) is the instrument to lead with}: a genuine,
  contiguous confidence interval, stable \(\hat\delta_1,\hat\delta_2\)
  across the whole search
\item
  \textbf{\(\tilde{\mathcal W}_{it-2}\) confirms the same qualitative
  story but is the weaker instrument}: real rejection pattern, a real
  interior optimum, but can only bound \(\lambda\) from above, not below
\item
  \textbf{No formal joint confidence region for
  \((\lambda,\delta_1,\delta_2)\) yet} --- next step, to get a stronger
  justification for one fixed \(\hat\theta\) to carry into the
  counterfactual
\end{itemize}
\end{frame}
...
\begin{center}
\includegraphics[width=\linewidth,height=0.9\textheight,keepaspectratio]{../Paper/images/1280-pct-change-from-prev.png}
\end{center}
\end{frame}

\begin{frame}{Headline Result}
\protect\phantomsection\label{headline-result}
At the current best-fit operating point (\(m^*_{it-1}\),
\(\hat\lambda=3.5\times10^{-7}\), \(\hat\delta_1=4.3\),
\(\hat\delta_2=0.54\)) --- forward-simulated, not yet run through formal
test inversion:

\begin{itemize}
\tightlist
\item
  A 1\% cut in \(\tau_P\) raises real revenue \textbf{+3.5\%}
\item
  The next 1\% cut only buys \textbf{+0.8\%} more --- diminishing fast
\item
  A mere 0.1\% \emph{increase} costs \textbf{-3.1\%} --- a tenth of the
  move undoes more than the first full point of cutting bought
\end{itemize}
\end{frame}
...
across every firm:
\[\tilde e(\Delta)=\frac{e_i}{1+\Delta}+\frac{\Delta}{2\lambda(1+\Delta)}\]
\(1/\lambda\) is large, so that intercept dominates almost immediately
past \(\Delta=0\) --- the same shift for every firm, regardless of size.
\end{frame}

\begin{frame}{The Optimal Cut, Not the Corner}
\protect\phantomsection\label{the-optimal-cut-not-the-corner}
\begin{itemize}
\tightlist
\item
  Revenue keeps rising as \(\Delta\to-1\) mathematically --- no evasion
  left, government keeps 100\% of the sales tax --- but that's not a
  serious answer: ignores why a purchases-side credit exists at all
  (cascading taxation), and pushes past where \((\omega,\psi)\) can be
  taken as fixed
\item
  Marginal revenue of the next cut flattens to a mechanical floor by
  \(\Delta\approx-20\%\) to \(-25\%\) --- the smallest cut that already
  captures nearly all the behavioral benefit
\item
  Not yet backed by a confidence set --- point estimate only
\end{itemize}
\end{frame}
