...
Alternatively, if we are interested in also identifying the Markov process of productivity, we can use the orthogonality we get from @eq-prod-markov, 
...
Note that by assumption \ref{ass-non-ev}, $D^{\mathcal{E}}$ and the density of $\varepsilon$ are still identified. To recover tax evasion, we can form the analog of @eq-ob-ev,
