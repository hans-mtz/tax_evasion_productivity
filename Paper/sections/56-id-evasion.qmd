...
Here, intuitively, we are trading unobservables, the unobserved tax evasion for the output shock, just to make our lives easier. We could have use use directly $m^*$ since we know the distribution of tax evasion $e$, which is equivalent to the distribution of $f_{m^*|m}(m^*|m)$.
...
To learn $\alpha$ and $h$, we can use the orthogonality we get from @eq-prod-markov.
$$
\mathbb{E}[\eta_{it}|k_{it},l_{it},k_{it-1},l_{it-1},\mathcal{W}_{it-1}]=0.
$$ {#eq-eta-moments}

Orthoganility follows from ($k_{it-1},l_{it-1},\mathcal{W}_{it-1}$) being known to the firm at period $t-1$, and ($k_{it},l_{it}$) being predetermined.
...
The first line follows from the orthogonality of the output shock $\mathbb{E}[\varepsilon_{it}|Z_{it}]=0$ and the law of iterated expectations. The second line follows from the definition of expectation and the independence of the output shock of the previous period. The third line follows from the interchangeability of integrals. Thus, $\alpha$ and $h$ are identified.

Also note that is it not necessary to learn tax evasion to recover productivity. This is useful if a practicioner is only interested in productivity and not on tax evasion.
