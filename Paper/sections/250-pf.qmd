...
```{r}
#| label: tbl-boot-pf
#| tbl-cap: Production function parameter estimates with bias-corrected bootstrap confidence intervals at the 5 percent significance level. 250 repetitions.
#| eval: false
#| include: false
...
#### Parametric Productivity Density 

Once, the production function parameters are known, we can recover $\omega$ by deconvolution using @eq-w-squiggle. Here I assume $\omega$ follows a normal distribution.
...
#### Non-Parametric Productivity Density

Nothing impedes us from estimating a non-parametric distribution of $\omega$. I deconvolute $\omega$ using penalized B-Splines from @eq-w-squiggle. First, I estimate the density of $\varepsilon$ using a kernel density estimator. 

To compare these productivity estimates to uncorrected ones, I simulate productivity using the B-Splines. To simulate 1000 independent draws from $\omega$, I numerically invert the B-Splines. First, I draw 1000 $u^S\sim U(0,1)$. Then I find the $\omega^S = \arg \min \left(\int_a^{\omega^S} s(t,\theta)dt-u^S\right)^2$. Then, I computed productivity, $\varphi_{it}^S=\exp\{\omega_{it}^S\}$. $\varepsilon$ is measurement error, so not part of productivity.

```{r}
#| label: tbl-np-prod
#| tbl-cap: Non-Parametric Deconvolution of $\omega$ using Penalized B-Splines. The error density function is estimated using a kernel density estimator fitted on the residuals of the first stage conditional on firms being Non-Evaders. 
...
omg_np_stats_df %>%
  mutate(
    ...
    instrument = case_when(
      grepl("lag_2_w_eps", rownames(.)) ~ "$\\mathcal{W}_{t-2}$",
      grepl("lag_m", rownames(.)) ~ "$m_{t-1}$",
      TRUE ~ " "
    ),
    .before = mean
  ) %>%
  arrange(sic_3, instrument) %>%
  kbl(
    digits = 3,
    table.attr = 'data-quarto-disable-processing="true"',
    row.names = FALSE
...
```{r}
#| label: tbl-np-sim-prod
#| tbl-cap: Summary statistics of the simulated productivity using the non-parametric estimates of $\omega$ and $\varepsilon$. Both functions were numerically inverted to obtain simulated draws of $\omega$. 1000 draws were used. Productivity is computed as $\varphi_{it}^S=\exp{\omega_{it}^S}$. $\varepsilon$ is measurement error, so not part of productivity.
