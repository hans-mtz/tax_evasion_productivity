...
I use @eq-big-w-squiggle assuming $\omega$ follows a normal distribution to deconvolute it from the output shock $\varepsilon$. @tbl-omega displays the estimates of the distribution parameters of $\omega$ assuming a parametric specification of an AR(1) and a third degree polynomial for the Markov process of $\omega$.
...
To compare my estimates of productivity without correcting for tax evasion, I simulated data by employing 1000 independent draws from $\omega$ and $\varepsilon$ using the estimated parameters in @tbl-omega. Then, I computed productivity, $\varphi_{it}^S=\exp\{\omega_{it}^S+\varepsilon_{it}^S\}$.
