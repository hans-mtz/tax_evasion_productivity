...
@eq-ob-ev suggest a way to test the presence of tax evasion through cost overreporting. Let $\mathbb{E}[\mathcal{V}_{it}]\equiv \mu_{\mathcal{V}}$ and define the null hypothesis as the absence of cost overreporting and the alternative hypothesis as the presence of cost overreporting. Consequently, to verify for the presence of tax evasion by overreporting, we can use a one-sided test where under the alternative hypothesis $H_1: \mu_{\mathcal{V}}>0$, and unde the null, $H_0: \mu_{\mathcal{V}}\le0$.

I use a bootstrap hypothesis test to verify the presence of tax evasion. @tbl-ielas-most-least-likely displays the estimated elasticities of intermediates for corporations and non-corporations for different industries. The table also displays the estimated value of the test statistic for the null hypothesis of no tax evasion.

I test the three least likely and the three most likely industries to evade taxes. The three least likely industries are exempt of sales taxes or lost exemption during the sample period. Exempt industries include 311 and 312 (Food Products), while industry 384 (Transport Equipment) lost exemption in 1984. Exempt industries are less likely to evade taxes because the sales tax rate is zero, so there is no incentive to overreport costs.

The three most likely industries have low purchase sales tax rates. Lower purchase sales tax rate imply higher incentives to overreport costs to claim additional deductions to the sales taxes owed to the government. These industries include 313 (Beverages), 331 (Wood Products except Furniture), and 369 (Non-Metallic Mineral Products).
...
Let $s_{it}=\ln\left(\frac{\rho_t M^*_{it}}{P_t Y_{it}}\right)$ and  $\bar s_G= \frac{1}{N_1T_1}\sum_{i=1}^{N_1}\sum_{t=1}^{T_1}s_{it}^G$ and $\bar s_B= \frac{1}{N_2T_2}\sum_{i=1}^{N_2}\sum_{t=1}^{T_2}s_{it}^B$ be the sample means for good guys (corporations) and the tax evasion guys, respectively. 
...
```{r}
#| label: tbl-ielas-most-least-likely
...
main_tbl %>% 
    filter(
        ...
        sic_3 %in% c(311, 312, 313, 369, 331)
    ) %>%
    mutate(
        across(n:share_imports_materials, ~replace(.x, type=="CI", NaN)),
        sic_3 = factor(sic_3, levels=c(311, 312, 313, 369, 331))
    ) %>%
    arrange(sic_3) %>%
    select(
    ...
    pack_rows(
        index = c(
            "2 Least Likely Industries" = 4,
            "3 Most Likely Industries" = 5
...
main_tbl %>% 
    ...
    mutate(
        across(n:share_imports_materials, ~replace(.x, type=="CI", NaN)),
        sic_3 = factor(sic_3, levels=top_10_revenue$sic_3[1:5])
    ) %>%
    select(
        sic_3, corps:mean_V_log_mats_share, n, n_Corp, sales, sales_sales_tax_rate:effective_sales_tax_rate_corrected
        # exporters:share_imports_materials
...
:::: {#note-remove-mle-deconv .content-hidden unless-meta="draft"}
...
[TODO: Add median and mode, remove skewness]{.content-hidden unless-meta="draft"}
