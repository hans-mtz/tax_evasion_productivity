...
The three mos likely industries have low purchase sales tax rates. Lower purchase sales tax rate imply higher incentives to overreport costs to claim additional deductions to the sales taxes owed to the government. These industries include 313 (Beverages), 331 (Wood Products except Furniture), and 369 (Non-Metallic Mineral Products).
...
main_tbl %>% 
    filter(
        ...
        sic_3 %in% c(311, 312, 384, 313, 369, 331)
    ) %>%
    mutate(
        across(n:share_imports_materials, ~replace(.x, type=="CI", NaN)),
        sic_3 = factor(sic_3, levels=c(311, 312, 384, 313, 369, 331))
    ) %>%
    arrange(sic_3) %>%
    select(
    ...
    pack_rows(
        index = c(
            "3 Least Likely Industries" = 6,
            "3 Most Likely Industries" = 6
