...
@tbl-CD-ar1-comparision and [-@tbl-CD-comparision] display the results of estimating the production function parameters following the identification strategy described above. In @tbl-CD-ar1-comparision, I use an AR(1) model for the Markov process of productivity and a third degree polynomial in [table @tbl-CD-comparision ]. In both cases, I employed $Z=\{k_{it},l_{it}\}$ as instruments.
...
#| tbl-cap: Cobb-Douglas production function parameters estimates, correcting intermediates —raw materials— for tax evasion vs. naive estimation and OLS. $h$ is an AR(1) process for 'Tax Evasion + GNR' but a third degree polynomial for 'GNR'. $Z=\\{k_{it},l_{it}\\}$.
#| eval: true
...
Assuming $\omega$ follows a normal distribution and @eq-w-squiggle, I deconvolute it from the output shock $\varepsilon$. @tbl-omega displays the estimates of the distribution parameters of $\omega$ assuming a parametric specification of an AR(1) and a third degree polynomial for the Markov process of $\omega$.
...
To compare productivity estimates corrected for tax-evasion to uncorrected, I simulated data by employing 1000 independent draws from $\omega$ and $\varepsilon$ using the parameters estimated for the 3D poly specification [@tbl-omega]. Then, I computed productivity, $\varphi_{it}^S=\exp\{\omega_{it}^S+\varepsilon_{it}^S\}$.
...
#| tbl-cap: Summary statistics of the productivity estimates using GNR correcting and not correcting for tax evasion. Tax-evasion corrected estimates were simulated using the parameters of @tbl-omega. 

load("../Code/Products/productivity.RData")

data.frame(
  rbind(
    cbind(Method="Tax Evasion + GNR", 
    productivity_tbl), 
    cbind(Method="GNR", CD_GNR_prod_tbl)
  )
)[,c(2,1,3:7)] %>%
arrange(
  inds, Method
...
The corrected productivity estimates have lower means and smaller standard deviations than uncorrected estimates. Industries 322 and 342 sell half or more of their output to retailers and directly to consumers (see @sec-inter).
