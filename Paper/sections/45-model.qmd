...
Note that firms can optimally choose not to evade, $e_{it}=0$. For these firms, any non-zero level of evasion may lead to a probability of detection close to one or excessive evasion costs. Corporations, for example, would rather not evade by overreporting their materials because lower profits would lead to a lower stock price. The cost of evading is too high for these firms. Furthermore, the tax authority in Colombia holds corporations to higher levels of scrutiny, so the probability of detection for these firms is also higher.

Their optimization problem is, therefore, the standard cost minimization problem. The optimal solution of these firms is well known. In particular, the optimal input choice of materials satisfies the following first-order condition
...
The system of equations solve the optimal material input choice and the optimal evasion decision.
