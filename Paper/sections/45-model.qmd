...
The tax authority audits firms at random and always detects misreporting if the firm engages in tax evasion and gets audited. The probabiliy of detection is then, $q(e_{it},a)$; where $a$ represents the exogenous monitoring effort by the tax authority. The probability of detection is continuous and increasing in both of its arguments. The more a firm overreports, the more likely it is to be detected. Likewise, the larger the efforts of the tax authority in monitoring suspicious behaviour of firms, the higher the probability of detection.
...
To better understand the incentives to overreport inputs, we can rewrite @eq-foc-cont-e in terms of the evasion elasticity of the probability of detection, $\xi_{q,e}=\frac{\partial q(e_{it}, a)}{\partial e_{it}}\frac{e_{it}}{q(e_{it}, a)}$, as follows,

$$
\frac{\rho_t}{P_t}\tau\bigg(1-(1+\phi)\left(1+\xi_{q,e}\right)q(e_{it}, a)\bigg) =\kappa_i'(e_{it})
$$ {#eq-foc-cont-e}

The left-hand side of @eq-foc-cont-e is the marginal benefit of tax evasion, while, the right-hand side is the marginal cost of evasion. The marginal benefit of evasion decreases with the evasion penalty $\phi$, the probability of detection $q(e_{it}, a)$, and the evasion elasticity of the probability of detection $\xi_{q,e}$. The marginal cost of evasion increases with the level of evasion because $\kappa'(e_{it})\ge0$.

Note that there could be firms that can optimally choose not to evade, $e_{it}=0$. It could be that @eq-foc-cont-e may not hold for some firms. Namely, there could be firms, that the marginal cost is greater than the marginal benefit for any non-zero level of tax evasion. Corporations, for example, would rather not evade by overreporting their materials because lower profits would lead to a lower stock price. The marginal cost of evasion is greater than the marginal benefit for any level. For these firms their optimization problem is, therefore, the standard cost minimization problem. The optimal solution of these firms is well known is also given by @eq-foc-m and $e_{it}=0$.
