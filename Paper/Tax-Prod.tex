...
\(e_{it}\equiv M^*-M\ge0\) to claim additional tax deductions and
maximize expected after-tax profits. \(M^*\) is the reported level of
materials, while \(M\) is the true level of materials.

Firms pay a tax rate \(\tau\) on their revenues after deducting their
materials costs. \(\tau\) is, therefore, equivalent to a value-added tax
(VAT) or corporate income tax (CIT), where firms can only claim
deductions for their expenses on materials.

The tax authority audits firms at random and always detects misreporting
if the firm engages in tax evasion and gets audited. The probabiliy of
detection is then, \(q(e_{it},a)\); where \(a\) represents the exogenous
monitoring effort by the tax authority. The probability of detection is
continuous and increasing in both of its arguments. The more a firm
overreports, the more likely it is to be detected. Likewise, the larger
the efforts of the tax authority in monitoring suspicious behaviour of
firms, the higher the probability of detection.

The evasion cost \(\kappa_i(e_{it})\) is increasing,
\(\kappa_i(e_{it})'>0\), and convex in the amount overreported,
\(\kappa''_i(e_{it})\ge0\). In addition, not overreporting is costless,
\(\kappa_i(0)=0\). This cost represents the direct cost of overreporting
such as acquiring fake invoices and resource cost of maintaining
parallel accounting books to keep track of the true and reported costs.
It might also represent indirect costs or losses of incurring in tax
evasion. If the firms decides to evade, the evading cost is paid whether
the firms is detected evading or not.

Finally, if tax evasion is detected, the tax authority imposes a
penalty. Detected firms are fined with a rate of \(\phi\) over the value
of evaded taxes in addition to the true tax liability.
...
\begin{aligned}
  ...
  &-\kappa_i(e_{it})-\phi\frac{\rho_t}{P_t}\tau e_{it}\bigg] \\
...
\begin{aligned}
  ...
  &+[1-q(e_{it}, a)]\left(\frac{\rho_{t}}{P_t}\tau e_{it}\right)
  -q(e_{it},a)\phi\frac{\rho_t}{P_t}\tau e_{it}-\kappa_i(e_{it}) \\
  \text{s.t. }\; Y_{it}&=K_{it}^{\alpha_k}L_{it}^{\alpha_L}M_{it}^{\beta}\exp(\omega_{it}+\varepsilon_{it})
\end{aligned}
}\end{equation}
...
\begin{equation}\phantomsection\label{eq-foc-m}{
...
\frac{\rho_t}{P_t}\tau\left(1-(1+\phi)\left(q(e_{it}, a)+\frac{\partial q(e_{it}, a)}{\partial e_{it}} e_{it}\right)\right) =\kappa_i'(e_{it})
}\end{equation}

where \(\mathcal{E}=\mathbb{E}[\exp(\varepsilon_{it})]\).

Under random auditing, Equation~\ref{eq-foc-m} is the familiar
optimality condition, where the left-hand side is the marginal product
of materials and the right-hand side is the relative prices of
materials.

To better understand the incentives to overreport inputs, we can rewrite
Equation~\ref{eq-foc-cont-e} in terms of the evasion elasticity of the
probability of detection,
\(\xi_{q,e}=\frac{\partial q(e_{it}, a)}{\partial e_{it}}\frac{e_{it}}{q(e_{it}, a)}\),
as follows,

\begin{equation}\phantomsection\label{eq-foc-cont-e}{
\frac{\rho_t}{P_t}\tau\bigg(1-(1+\phi)\left(1+\xi_{q,e}\right)q(e_{it}, a)\bigg) =\kappa_i'(e_{it})
}\end{equation}
...
of evasion. The marginal benefit of evasion decreases with the evasion
penalty \(\phi\), the probability of detection \(q(e_{it}, a)\), and the
evasion elasticity of the probability of detection \(\xi_{q,e}\). The
marginal cost of evasion increases with the level of evasion because
\(\kappa'(e_{it})\ge0\).

Note that there could be firms that can optimally choose not to evade,
\(e_{it}=0\). It could be that Equation~\ref{eq-foc-cont-e} may not hold
for some firms. Namely, there could be firms, that the marginal cost is
greater than the marginal benefit for any non-zero level of tax evasion.
Corporations, for example, would rather not evade by overreporting their
materials because lower profits would lead to a lower stock price. The
marginal cost of evasion is greater than the marginal benefit for any
level. For these firms their optimization problem is, therefore, the
standard cost minimization problem. The optimal solution of these firms
is well known is also given by Equation~\ref{eq-foc-m} and \(e_{it}=0\).
...
\begin{fbx}{Assumption}{Assumption 2.1: }{Non-Evaders}
\phantomsection\label{ass-non-ev}
...
\begin{fbx}{Assumption}{Assumption 2.2: }{Independence}
\phantomsection\label{ass-ind}
...
Assumption \hyperref[ass-ind]{2.2} implies that input overreporting is
...
\hyperref[ass-non-ev]{2.1} to recover the production function parameter
\(\beta\)
...
\hyperref[ass-ind]{2.2}, the tax evasion, \(\varepsilon_{it}\) ,is
...
\hyperref[ass-non-ev]{2.1}, we can identify the constant
...
Note that by assumption \hyperref[ass-non-ev]{2.1}, \(D^{\mathcal{E}}\)
...
\hyperref[ass-non-ev]{2.1}, however, when we form the analogous of
...
shock \(\varepsilon\) are independent {[}\hyperref[ass-ind]{2.2}{]} with
...
\(\mathbb{E}[\mathcal{V}_{it}]\equiv \mu_{\mathcal{V}}\) and define the
null hypothesis as the absence of cost overreporting and the alternative
hypothesis as the presence of cost overreporting. . Consequently, to
verify for the presence of tax evasion by overreporting, we can use a
one-sided test where under the alternative hypothesis
\(H_1: \mu_{\mathcal{V}}>0\), and unde the null,
\(H_0: \mu_{\mathcal{V}}\le0\).

I use a bootstrap hypothesis test to verify the presence of tax evasion.
Table~\ref{tbl-ielas-most-least-likely} displays the estimated
elasticities of intermediates for corporations and non-corporations for
different industries. The table also displays the estimated value of the
test statistic for the null hypothesis of no tax evasion.

I test the three least likely and the three most likely industries to
evade taxes. The three least likely industries are exempt of sales taxes
or lost exemption during the sample period. Exempt industries include
311 and 312 (Food Products), while industry 384 (Transport Equipment)
lost exemption in 1984. Exempt industries are less likely to evade taxes
because the sales tax rate is zero, so there is no incentive to
overreport costs.

The three mos likely industries have low purchase sales tax rates. Lower
purchase sales tax rate imply higher incentives to overreport costs to
claim additional deductions to the sales taxes owed to the government.
These industries include 313 (Beverages), 331 (Wood Products except
Furniture), and 369 (Non-Metallic Mineral Products).

\begin{table}

\caption{\label{tbl-ielas-most-least-likely}Intermediate elasticities
for Corporations (Non-Evaders) and non-Corporations (Evaders) for
different industries. Estimates were obtained using GNR (2020) method
assuming a CD functional form. Intermediates are defined as raw
materials. \(\varepsilon\) is defined as \textbf{measurement error}. The
Tax Evasion Test displays the coefficient of the statistical test, where
the null hypothesis states that there is no tax evasion through cost
...
\hline
\multicolumn{10}{l}{\textbf{3 Least Likely Industries}}\\
\hline
...
\cline{1-10}
\multicolumn{10}{l}{\textbf{3 Most Likely Industries}}\\
\hline
...
\caption{\label{tbl-ielas-top5-1}Intermediate elasticities for
...
\textbf{?@tbl-evasion-test-CD} shows that the null hypothesis of no tax
...
\textbf{?@tbl-CD-ar1-comparision} displays the results of estimating the
production function parameters following the identification strategy
...
table \textbf{?@tbl-CD-ar1-comparision}.
...
\textbf{?@tbl-CD-ar1-comparision} shows that correcting for tax evasion
...
productivity using the B-Splines. To simulate 1000 independent draws
from \(\omega\), I numerically invert the B-Splines. First, I draw 1000
\(u^S\sim U(0,1)\). Then I find the
\(\omega^S = \arg \min \left(\int_a^{\omega^S} s(t,\theta)dt-u^S\right)^2\).
Then, I computed productivity, \(\varphi_{it}^S=\exp\{\omega_{it}^S\}\).
\(\varepsilon\) is measurement error, so not part of productivity.
...
\hline
 & \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 8.410 & 0.926 & -0.496\\
\cline{2-5}
\multirow[t]{-2}{*}{\raggedright\arraybackslash 313} & \$m\_\{t-1\}\$ & 3.799 & 0.454 & -0.088\\
\cline{1-5}
 & \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 2.885 & 0.378 & -0.069\\
\cline{2-5}
\multirow[t]{-2}{*}{\raggedright\arraybackslash 321} & \$m\_\{t-1\}\$ & 2.885 & 0.391 & -0.127\\
\cline{1-5}
 & \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 4.102 & 0.297 & -0.481\\
\cline{2-5}
\multirow[t]{-2}{*}{\raggedright\arraybackslash 322} & \$m\_\{t-1\}\$ & 4.253 & 0.303 & -0.455\\
\cline{1-5}
 & \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 4.616 & 0.269 & -0.179\\
\cline{2-5}
\multirow[t]{-2}{*}{\raggedright\arraybackslash 331} & \$m\_\{t-1\}\$ & 4.140 & 0.256 & -0.724\\
\cline{1-5}
 & \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 3.017 & 0.305 & -0.489\\
\cline{2-5}
\multirow[t]{-2}{*}{\raggedright\arraybackslash 369} & \$m\_\{t-1\}\$ & -3.984 & 1.929 & 0.542\\
\hline
\end{tabular}
...
simulated draws of \(\omega\). 1000 draws were used. Productivity is
computed as \(\varphi_{it}^S=\exp{\omega_{it}^S}\). \(\varepsilon\) is
measurement error, so not part of productivity.}
...
\hline
 & GNR & 60.8 & 30.7 & 43.3 & 55.3 & 69.6\\
\cline{2-7}
 & TE: \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 6253.5 & 5744.5 & 2697.1 & 4867.0 & 7690.4\\
\cline{2-7}
\multirow[t]{-3}{*}{\raggedleft\arraybackslash 313} & TE: \$m\textasciicircum{}*\_\{t-1\}\$ & 48.2 & 25.1 & 35.6 & 44.9 & 54.9\\
\cline{1-7}
 & GNR & 21.0 & 11.2 & 13.8 & 18.7 & 25.5\\
\cline{2-7}
 & TE: \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 18.9 & 8.7 & 14.8 & 17.4 & 20.8\\
\cline{2-7}
\multirow[t]{-3}{*}{\raggedleft\arraybackslash 321} & TE: \$m\textasciicircum{}*\_\{t-1\}\$ & 18.9 & 8.8 & 14.8 & 17.5 & 21.0\\
\cline{1-7}
 & GNR & 34.9 & 16.4 & 24.0 & 33.4 & 43.2\\
\cline{2-7}
 & TE: \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 60.9 & 17.3 & 51.6 & 60.0 & 68.8\\
\cline{2-7}
\multirow[t]{-3}{*}{\raggedleft\arraybackslash 322} & TE: \$m\textasciicircum{}*\_\{t-1\}\$ & 71.0 & 20.8 & 59.7 & 69.8 & 80.3\\
\cline{1-7}
 & GNR & 51.3 & 26.1 & 33.2 & 46.4 & 62.7\\
\cline{2-7}
 & TE: \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 103.1 & 31.4 & 85.9 & 99.6 & 115.2\\
\cline{2-7}
\multirow[t]{-3}{*}{\raggedleft\arraybackslash 331} & TE: \$m\textasciicircum{}*\_\{t-1\}\$ & 63.8 & 17.8 & 55.4 & 63.0 & 70.9\\
\cline{1-7}
 & GNR & 34.5 & 26.9 & 17.0 & 29.2 & 45.5\\
\cline{2-7}
 & TE: \$\textbackslash{}mathcal\{W\}\_\{t-2\}\$ & 20.3 & 4.6 & 18.1 & 20.0 & 22.0\\
\cline{2-7}
\multirow[t]{-3}{*}{\raggedleft\arraybackslash 369} & TE: \$m\textasciicircum{}*\_\{t-1\}\$ & 0.2 & 1.2 & 0.0 & 0.0 & 0.0\\
\hline
\end{tabular}
...
\includegraphics{Tax-Prod_files/figure-pdf/unnamed-chunk-46-1.pdf}

\includegraphics{Tax-Prod_files/figure-pdf/unnamed-chunk-46-2.pdf}

\includegraphics{Tax-Prod_files/figure-pdf/unnamed-chunk-46-3.pdf}

\includegraphics{Tax-Prod_files/figure-pdf/unnamed-chunk-46-4.pdf}

\includegraphics{Tax-Prod_files/figure-pdf/unnamed-chunk-46-5.pdf}
